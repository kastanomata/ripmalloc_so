...
TimingResult get_real_timing(TestFunction test_fn, const char* test_name) {
    ...
    // Calculate averages
    result.user_time = total.user_time / 10;
    result.system_time = total.system_time / 10;
    result.real_time = total.real_time / 10;
    
    printf("\nTiming results for %s (average of 10 runs):\n", test_name);
    ...
    if (failed_runs > 0) {
        printf("  Warning: %d out of 10 test runs returned non-zero status\n", failed_runs);
    }
